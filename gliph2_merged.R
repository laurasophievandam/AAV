...
# Fisher_score, vb_score, number_subject, final_score.
...
summarize_dominant_value <- function(df, value_col, group_col = "index") {
  dominant <- df %>%
    ...
    ungroup() %>%
    select(-n) %>%
    rename(!!paste0(value_col, "_max") := all_of(value_col))
...
count_antigen_hits <- function(df, antigen_col = "Virus", group_col = "index") {
  dominant <- df %>%
    ...
    slice_max(n, n = 1, with_ties = FALSE) %>%
    ungroup() %>%
    rename(dominant_antigen = all_of(antigen_col), dominant_antigen_n = n)
...
#' @param color_attr name of the vertex attribute the network was colored
#'   by (must match the `node_attr_cols` name passed as `color_by` in
#'   build_specificity_network(), default "type")
plot_specificity_network <- function(graph, output_pdf,
  ...
  # pair category/color as distinct rows, not two independent unique()
  # calls, so the legend entries can't drift out of correspondence
  legend_pairs <- unique(vert[, c(color_attr, "color")])
  legend(x = -1.5, y = 1, legend = legend_pairs[[color_attr]],
         col = legend_pairs$color, pch = 19, pt.cex = 1, bty = "n",
